...
class _EditEntryState extends State<EditEntry> {
  ...
  void initState() {
    ...
    _journalEdit.journal = widget.journalEdit.journal; if (widget.add) {
  ...
  Future<DateTime?> _selectDate(DateTime selectedDate) async {
    ...
    final DateTime? _pickedDate = await showDatePicker( context: context,
      initialDate: _initialDate,
      firstDate: DateTime.now().subtract(Duration(days: 365)), lastDate: DateTime.now().add(Duration(days: 365)),
    );

    if (_pickedDate != null) { selectedDate = DateTime(
        _pickedDate.year,
        _pickedDate.month,
        _pickedDate.day,
        _initialDate.hour,
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          children: <Widget>[
            FlatButton(
              padding: EdgeInsets.all(0.0), child: Row(
              children: <Widget>[
                Icon(
                  Icons.calendar_today, size: 22.0,
                  color: Colors.black54,
                ),
                SizedBox(width: 16.0,), Text(DateFormat.yMMMEd().format(_selectedDate),
                  style: TextStyle(
            ...
            Row(
              ...
              children: <Widget>[
                FlatButton(
                  child: Text('Cancel'), color: Colors.grey.shade100, onPressed: () {
